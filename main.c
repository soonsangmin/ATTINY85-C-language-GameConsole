...
#define SSD1306_SCL PORTB4 // SCL, Pin 4 on SSD1306 Board- for webbogles
board
#define SSD1306_SDA PORTB3 // SDA, Pin 3 on SSD1306 Board- for webbogles
    board
...
void loop()
{
    ...
    // The lower case character set is seriously compromised because I've had to truncate the
 ASCII table
 // to release space for executable code- hence lower case y and w are remapped to h and /
 respectively.
 // There is no z in the table (or h!) as these aren't used anywhere in the text here and most
 of the
 // symbols are also missing for the same reason (see my hacked version of font6x8.h
font6x8AJ.h for more detail)
...
void ssd1306_init(void)
{
    ...
    ssd1306_send_command(0x80); // 0xa4,Output follows RAM content;0xa5,Output
    ignores RAM content
...
void playSpaceAttack()
{
    ...
    while (stopAnimate == 0)
    {
        while (1)
        {
            ...
            // deal with inputs
            if (analogRead(0) < 940)
                fire = 1; // this reads the reset pin (pin 1) of the Attiny85- to use
            it, see comments section above if (digitalRead(2) == 1)
            {
                if (digitalRead(0) == 1)
                    fire = 1;
                        ...
                        if (inc == lastActiveRow)
                        {
                            ...
                            { // this alien is going to fire!
                                ...
                                {                                                                              // we've found a slot
                                    ...
                                    alienFire[afIndex][1] = ((positionNow * 8) + ((bl - firstAlien) * 8) + 4); // x
                                    position
            ...
            // Display the score
            doNumber(0, 6, score);
            // Burn clock cycles to keep game at constant (ish) speed when there are low numbers of
            live aliens int burnLimit = (8 - (lastAlien - firstAlien));
            ...
            { // fire has been pressed and we're not currently
                firing - initiate fire !!playerFire[0] = 1;
            ...
            if (firecounter >= 2)
            {
                ...
                if ((fire == 1))
                {
                    ...
                    if ((playerFire[1] >= leftLimit) && (fireYidx >= 0) && (fireYidx <= lastActiveRow) && (fireXidx >= 0) && (fireXidx < 9))
                    {
                        ...
                        {
                            int lastActiveToClear = lastActiveRow; // if we kill the last alien on a row- we still
                            need to clear that row(end of this fn) if (fireYidx == 2) deadOn2++;
                            if (fireYidx == 1)
                                deadOn1++;
                            if (deadOn2 == 5)
                            {
                            ...
                            { // this is the first Alien- sweep back and reset to new
                                first alien for (int xi = lastAlien; xi >= firstAlien; xi--)
                                {
                                    if ((row[0][xi] == 1) | (row[1][xi] == 1) | (row[2][xi] == 1))
                                        newFirst = xi;
                                }
                            ...
                            { // this is the last Alien- sweep thru and reset to new last
                                alien
                            }
                ...
                for (byte afIndex = 0; afIndex < 5; afIndex++)
                {
                    if (alienFire[afIndex][0] == 1)
                    {
                        ...
                        if (alienFire[afIndex][2] >= 56)
                        {
                            ...
                            if (
                                ((alienFire[afIndex][1]) > player) && ((alienFire[afIndex][1]) <
                                                                       player + platformWidth))
                            { // you've been hit!!
                                stopAnimate = 1;
                                goto die;
                            }
...
{
    // clearalienarea
    for (intinc = row; inc <= row + lastActive; inc++)
    {
        ssd1306_setpos(0, inc);
        ssd1306_send_data_start();
        for (intbl = 0; bl < 16; bl++)
        {
            sendBlock(0);
        }
        ssd1306_send_data_stop();
